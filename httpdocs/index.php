              ...
              if ($rezultat= $connect->query("SELECT * FROM stan WHERE co = 0"))
		            {
			               $wierszy = $rezultat->num_rows;
			                  if($wierszy>0) {
                                ...
                                if ($wiersz['stan'] == "1") {
                                  echo  "<div class=\"semcz\" id=\"".$wiersz['nr']."\" onclick=\"sem(this.id)\">".$wiersz['nr']."</div>";
                                }
                                elseif ($wiersz['stan'] == "0") {
                                  echo  "<div class=\"sem\" id=\"".$wiersz['nr']."\" onclick=\"sem(this.id)\">".$wiersz['nr']."</div>";
                                }
              ...
              if ($rezultat= $connect->query("SELECT * FROM stan WHERE co = 1"))
                {
                     $wierszy = $rezultat->num_rows;
                        if($wierszy>0) {
                              while ($wiersz = $rezultat->fetch_assoc()) {
                                if ($licznik == 0) {
                                  echo "<div class=\"contener\" id=\"".$wiersz['grupa']."\" onclick=\"zwrot(this.id)\">";
                                }
                                if ($wiersz['stan'] == "0") {
                                  echo  "<div class=\"zw ".$wiersz['grupa']."\"  >".$wiersz['nr']."</div>";
                                }
                                elseif ($wiersz['stan'] == "1") {
                                  echo  "<div class=\"zwz ".$wiersz['grupa']."\"  >".$wiersz['nr']."</div>";
                                }
                                if ($licznik == 0) {
                                  $licznik++;
                                }
